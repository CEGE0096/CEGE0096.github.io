...
from geometry import are_line_segments_equal


def read(input_file='are_line_segments_equal_sample'):
...
def test_are_line_segments_equal_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_line_segments_equal(*args)
