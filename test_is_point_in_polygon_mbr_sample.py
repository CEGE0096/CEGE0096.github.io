...
from geometry import is_point_in_polygon_mbr


def read(input_file='is_point_in_polygon_mbr_sample'):
...
def test_is_point_in_polygon_mbr_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = is_point_in_polygon_mbr(*args)
