...
from geometry import are_lines_intersecting_on_one_point


def read(input_file='are_lines_intersecting_on_one_point_sample'):
    with open(f'./tests/data/{input_file}.pkl', 'rb') as f:
        rows = pickle.load(f)
    return rows


def test_are_lines_intersecting_on_one_point_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_lines_intersecting_on_one_point(*args)
