...
from geometry import are_polygon_sides_in_order


def read(input_file='are_polygon_sides_in_order_sample'):
...
def test_are_polygon_sides_in_order_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_polygon_sides_in_order(*args)
