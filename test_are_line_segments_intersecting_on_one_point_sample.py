...
from geometry import are_line_segments_intersecting_on_one_point


def read(input_file='are_line_segments_intersecting_on_one_point_sample'):
...
def test_are_line_segments_intersecting_on_one_point_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_line_segments_intersecting_on_one_point(*args)
