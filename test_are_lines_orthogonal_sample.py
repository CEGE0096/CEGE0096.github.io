...
from geometry import are_lines_orthogonal


def read(input_file='are_lines_orthogonal_sample'):
...
def test_are_lines_orthogonal_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_lines_orthogonal(*args)
