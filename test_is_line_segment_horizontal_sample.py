...
from geometry import is_line_segment_horizontal


def read(input_file='is_line_segment_horizontal_sample'):
...
def test_is_line_segment_horizontal_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = is_line_segment_horizontal(*args)
