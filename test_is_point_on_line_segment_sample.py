...
from geometry import is_point_on_line_segment


def read(input_file='is_point_on_line_segment_sample'):
...
def test_is_point_on_line_segment_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = is_point_on_line_segment(*args)
