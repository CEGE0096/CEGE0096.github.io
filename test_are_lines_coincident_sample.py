...
from geometry import are_lines_coincident


def read(input_file='are_lines_coincident_sample'):
...
def test_are_lines_coincident_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_lines_coincident(*args)
