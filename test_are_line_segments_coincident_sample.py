...
from geometry import are_line_segments_coincident


def read(input_file='are_line_segments_coincident_sample'):
    with open(f'./tests/data/{input_file}.pkl', 'rb') as f:
        rows = pickle.load(f)
    return rows


def test_are_line_segments_coincident_sample():
    rows = read()
    for row in rows:
        args, res = row[:-1], row[-1]
        res_hat = are_line_segments_coincident(*args)
